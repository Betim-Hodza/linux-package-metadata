...
def process_package(package_url: str):
    ...
    # Download package and write package to disk
    response = requests.get(package_url, stream=True)
    with open(package_file, "wb") as f:
        for chunk in response.iter_content(chunk_size=1024):
            f.write(chunk)
    
    # extract nbame and version for ubuntu (name_version_arch.deb)
    package_name = package.split("_")[0]
    package_version = package.split("_")[1]

    # create a temp package dir
...
def main():
    # create output and temp dirs to do our work in
